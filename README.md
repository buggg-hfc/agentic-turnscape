...
- The web turn panel now supports fixed action cards plus a freeform action composer. Freeform player text is accepted as a `custom` action, previewed as inferred intent/risk/target chips, then adjudicated by the same 2d6/referee/state-patch path instead of directly mutating world state.
