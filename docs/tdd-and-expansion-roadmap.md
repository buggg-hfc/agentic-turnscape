...
- First-wave expansion packs can now run deterministic three-day playthroughs through the same Agent/referee/state-patch pipeline. Successful runs advance scenario stability clocks, reach success endings, and can be replayed from their emitted `state_patch` records.
